...
public class DownloadAuthorizationTest {
  ...
  @Test
  public void test_authorization_ok_open_no_scopes_expired_token() {
    val sut = getSut(OPEN_ACCESS);
  ...
  public void test_authorization_fail_controlled_right_project_wrong_operation() {
    val sut = getSut(CONTROLLED_ACCESS);
    val authentication = getAuthentication(false, "test1.download", "test.OTHER-CODE.upload",
      "test." + TEST_PROJECT + ".upload", "test.CODE.upload", "test2.download");
    val result = sut.authorize(authentication, TEST_OBJECT_ID);
    assertFalse(result);
  }
  ...
  public DownloadScopeAuthorizationStrategy getSut(String accessType) {
    ...
    return new DownloadScopeAuthorizationStrategy(GLOBAL_SCOPE, metadataService);
  }
